...
VERSION=$(curl -sL https://github.com/krakendio/krakend-ce/releases | \
        grep -o 'releases/tag/v[0-9]*.[0-9]*.[0-9]*' | sort -V | \
        tail -1 | awk -F'/' '{ print $3}')


curl -sLo checksums.txt https://github.com/krakendio/krakend-ce/releases/download/${VERSION}/checksums.txt
...
ALPINE_VERSION=$(curl -sL https://raw.githubusercontent.com/krakendio/krakend-ce/${VERSION}/Makefile | \
